...
context = LibSecp256k1.context_create(SECP256K1_CONTEXT_NONE)

unless LibSecp256k1.context_randomize(context, randomness)
...
secret_key = loop {
  ...
  if LibSecp256k1.ec_seckey_verify context, value
    break value
  end
}
...
unless LibSecp256k1.ec_pubkey_create(context, pointerof(public_key), secret_key)
...
unless LibSecp256k1.ec_pubkey_serialize(context, compressed_public_key, pointerof(size), pointerof(public_key), SECP256K1_EC_COMPRESSED)
